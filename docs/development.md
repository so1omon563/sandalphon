...
count. Protocol revisions 2 and 3 may additionally report a prior bounded
failure, the numeric renderer target count, and the numeric canonical-page
count. They never return the underlying process, signature, renderer, target
metadata, or task payload that caused a rejection.
